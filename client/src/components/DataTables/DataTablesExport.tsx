...
export interface DataTablesExportItem {
  id: number;
  name: string;
  image: string;
  position: string;
...
interface DataTablesExportProps {
  data: DataTablesExportItem[];
  title?: string;
  enableSelection?: boolean;
  onSelectionChange?: (selectedItems: DataTablesExportItem[]) => void;
  onBulkAction?: (action: string, selectedItems: DataTablesExportItem[]) => void;
  onEdit?: (item: DataTablesExportItem) => void;
  onDelete?: (item: DataTablesExportItem) => void;
  onDownload?: () => void;
}

const DataTablesExport: React.FC<DataTablesExportProps> = ({ 
  data, 
  title = "Datatable 3",
    ...
    return data
      .filter(
        (person) =>
          ...
          person.salary.toLowerCase().includes(searchLower) ||
          (person.status && person.status.toLowerCase().includes(searchLower))
      )
      .sort((a, b) => {
        let modifier = sortDirection === "asc" ? 1 : -1;
        const aValue = a[sortColumn as keyof DataTablesExportItem];
        const bValue = b[sortColumn as keyof DataTablesExportItem];
  ...
  const isIndeterminate = paginatedData.some(item => selectedItems.has(item.id)) && !isAllSelected;
        ...
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
          <div className="mb-4 flex flex-col gap-2 px-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                onClick={onDownload}
                className="flex items-center gap-2 rounded-lg bg-blue-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:bg-blue-600 dark:hover:bg-blue-700"
              >
          ...
          <div className="max-w-full overflow-x-auto">
            <div className="min-w-[1102px]">
              ...
              <div className="grid grid-cols-12 border-t border-gray-200 dark:border-gray-800">
                ...
                <div className={`${enableSelection ? 'col-span-3' : 'col-span-4'} flex items-center border-r border-gray-200 px-4 py-3 dark:border-gray-800`}>
                  ...
                  {enableSelection && (
                    <div className="col-span-1 flex items-center border-r border-gray-100 px-4 py-[17.5px] dark:border-gray-800">
                      <input
                        ...
                        onChange={(e) => handleSelectItem(person.id, e.target.checked)}
                        className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:ring-offset-gray-800 dark:focus:ring-blue-600"
                      />
                    </div>
                  )}
                  <div className={`${enableSelection ? 'col-span-3' : 'col-span-4'} flex items-center border-r border-gray-100 px-4 py-[17.5px] dark:border-gray-800`}>
                    <div className="flex items-center gap-3">
...
export default DataTablesExport;