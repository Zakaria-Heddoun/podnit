...
const ProductDataTable: React.FC<ProductDataTableProps> = ({ 
  ...
  // Handler functions for toggle switches
  const handleStockToggle = (productId: number, checked: boolean) => {
    setProductData(prevData => 
      prevData.map(product => 
        product.id === productId 
          ? { ...product, stock: checked ? "100" : "10" }
          : product
      )
    );
  };

  const handleActiveToggle = (productId: number, checked: boolean) => {
    setProductData(prevData => 
      prevData.map(product => 
        product.id === productId 
          ...
          <div className="w-full overflow-x-auto">
            <div className="w-full">
              ...
              <div className="grid grid-cols-11 border-t border-gray-200 dark:border-gray-800">
                ...
                <div className="col-span-2 flex items-center border-r border-gray-200 px-4 py-3 dark:border-gray-800">
                  <div className="flex w-full cursor-pointer items-center justify-between" onClick={() => sortBy('stock')}>
              ...
              {paginatedData.map((product) => (
                <div key={product.id} className="grid grid-cols-11 border-t border-gray-100 dark:border-gray-800">
                  ...
                  <div className="col-span-2 flex items-center justify-center border-r border-gray-100 px-4 py-[17.5px] dark:border-gray-800">
                    <Switch
                      key={`stock-${product.id}-${product.stock}`}
                      label=""
                      defaultChecked={parseInt(product.stock) > 50}
                      onChange={(checked) => handleStockToggle(product.id, checked)}
                      color="green"
                    />
                  </div>
                  <div className="col-span-2 flex items-center border-r border-gray-100 px-4 py-[17.5px] dark:border-gray-800">
                  ...
                  <div className="col-span-2 flex items-center justify-center border-r border-gray-100 px-4 py-[17.5px] dark:border-gray-800">
                    <Switch
                      ...
                      onChange={(checked) => handleActiveToggle(product.id, checked)}
                      color="green"
                    />
                  </div>
